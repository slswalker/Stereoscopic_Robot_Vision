...
//  TestOpenCV
//
//  Created by Sam Walker on 3/22/12.
//  Copyright (c) 2012 Sam Walker. All rights reserved.
//

#ifndef TestOpenCV_Calibrate_h
#define TestOpenCV_Calibrate_h

//#include "Constants.h"

//void showCapture(CvCapture capture)
//{
//    const char* camera = "show";
...
static void showDepthMap(CvMat *Q, CvMat *mx1, CvMat *my1, CvMat *mx2, CvMat *my2, CvSize imageSize,
                         CvCapture* left, CvCapture* right, const char* camLeft, const char* camRight)
{
    IplImage* img1= cvQueryFrame(left);
    IplImage* img2= cvQueryFrame(right);
...
static void
StereoCalibPictures(int nx, int ny, int useUncalibrated, IplImage* imgArr[PICTURES], int save)
{    
    using namespace std;
    using namespace cv;
    ...
    int i, j, lr, nframes, n = nx*ny, activeVar = 0, N = 0;
...
//                                              30, 0.01) );
    ...
    if( showUndistorted )
    {
        ...
        BMState->textureThreshold=10;
        BMState->uniquenessRatio=15;
        printf("IsVerticalStereo: %d\n",isVerticalStereo);
        for( i = 0; i < nframes; i++ )
        {
            IplImage* img1=imgArr[i++];
            IplImage* img2=imgArr[i++];
            if( img1 && img2 )
            {
                CvMat part;
                cvRemap( img1, img1r, mx1, my1 );
...
static void
StereoCalibFile(const char* imageList, int nx, int ny, int useUncalibrated, int save)
{    
    using namespace std;
    using namespace cv;
    int displayCorners = 0;
